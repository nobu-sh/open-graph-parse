...
const mediaSetup = (ogObject) => {
  ...
  if (ogImages.length) ogObject.ogImage = ogImages[0]
  if (ogVideos.length) ogObject.ogVideo = ogVideos[0]
  if (twitterImages.length) ogObject.twitterImage = twitterImages[0]
  if (twitterPlayers.length) ogObject.twitterPlayer = twitterPlayers[0]
  if (musicSongs.length) ogObject.musicSong = musicSongs
